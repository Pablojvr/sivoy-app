...
async function migrate() {
    try {
        ...
        console.log('Updating horarios_operativos...');
        await pool.query('ALTER TABLE horarios_operativos ADD COLUMN agencia_id_new INTEGER;');
        await pool.query('UPDATE horarios_operativos h SET agencia_id_new = a.id FROM agencias a WHERE h.agencia_id = a.id_destino;');
        ...
        console.log('Updating reglas_entrega...');
        await pool.query('ALTER TABLE reglas_entrega ADD COLUMN agencia_id_new INTEGER;');
        await pool.query('UPDATE reglas_entrega r SET agencia_id_new = a.id FROM agencias a WHERE r.agencia_id = a.id_destino;');
