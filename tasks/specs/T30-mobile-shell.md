...
3. **T30d3: Purga de Búsqueda Legada (Limpieza Final):**
   ...
   - **T30d3a — handlers hoja:** aceptado por Codex. Se eliminaron 15 handlers públicos sin entrada desde plantilla, router ni consumidores externos y dos flags que quedaron sin lecturas. Los callbacks `showOriginPinDetails` y `showDestinoPinDetails` se conservaron al comprobarse su uso desde marcadores. El shell pasó de 2307 a 2128 líneas; 232 pruebas, build, diff-check y render móvil en `127.0.0.1:4303` fueron validados. El único error de consola observado corresponde a la API local no disponible, no a la compilación o renderizado del frontend.
   - **T30d3b1 — selector público duplicado:** aceptado por Codex. Se retiraron 20 métodos internos del selector/modal que ya pertenecen a `HomeComponent`, junto con su prueba de implementación obsoleta. `closeLocationSelector` se conservó temporalmente porque la rama legada de destino personalizado aún la consume. El shell quedó en 1831 líneas; 231 pruebas, build, diff-check y render local fueron validados.
   - **T30d3b2 — motor y estado legados:** aceptado por Codex. Se eliminaron el cálculo HTTP duplicado de rutas, discovery, destino personalizado por click y 29 campos que solo pertenecían a esos flujos. Se preservaron disponibilidad horaria, geolocalización, mapa, rutas destacadas y preview administrativo. El shell quedó en 1313 líneas; 231 pruebas, build, diff-check y el recorrido local inicio → buscar destino → volver a inicio fueron validados.
...
**Resultado T30:** completado. `MobileAppComponent` ya no posee la búsqueda pública ni consulta rutas; actúa como shell de navegación/mapa y conserva temporalmente wiring administrativo fuera de alcance. Reducción final: 2326 → 1313 líneas (43.6%).
