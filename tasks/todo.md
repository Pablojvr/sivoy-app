...
- [ ] T46 Separar superficie pública y operativa; endurecer HTTP y pool.
  - Spec en ejecución: `tasks/specs/T46-http-pool-boundary.md`. T46a configura límites y timeout de adquisición. T46b1 expone cierre idempotente del pool. T46b2a añade eventos seguros; T46b2b integra SIGTERM/SIGINT solo en autoarranque, espera HTTP antes de DB, con timeout y pruebas RED→GREEN. T46c: el usuario aprobó desactivar temporalmente las escrituras de empresas y puntos solo en producción; las cuatro rutas están inventariadas, sin login por ahora.
